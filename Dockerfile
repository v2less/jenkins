...
# 安装 常用软件
USER root
RUN sed -i "s@http://.*debian.org@http://mirrors.tuna.tsinghua.edu.cn@g" /etc/apt/sources.list.d/debian.sources
...
# 安装 Docker Compose
RUN curl -L https://mirror.ghproxy.com/https://github.com/docker/compose/releases/download/latest/docker-compose-$(uname -s)-$(uname -m) -o /usr/local/bin/docker-compose
RUN chmod +x /usr/local/bin/docker-compose && \
    usermod -aG docker jenkins && \
    usermod -u 1004 jenkins && \
    groupmod -g 1004 jenkins
...
ENV JAVA_OPTS="-Djava.awt.headless=true -Duser.timezone=Asia/Shanghai -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -Djenkins.install.runSetupWizard=true"
